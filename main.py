...
def setup_dataloaders(args, tokenizer=None):
    ...
    if(not 'extract_model' in args):
        ...
        # classifier sees valid/test features in the same basis it was trained in.
        # --per_split_whitening restores the old per-split fitting.
        shared_whitening = None
        if not args.get('per_split_whitening'):
            shared_whitening = getattr(train_dataset, 'whitening_params', None)
